...
describe("EPIC 1.4: Edit Profile", () => {
  ...
  before(async () => {
    ...
    await browser.waitUntil(
      async () =>
        (await browser.execute(() => localStorage.getItem("userInfo"))) !==
        null,
...
it("EP01.4_01 - Update profile successfully with valid data", async () => {
    await ProfilePage.clearAndType(ProfilePage.inputEmail, "newuser@gmail.com");
    await ProfilePage.clearAndType(ProfilePage.inputName, "New User Name");

    await ProfilePage.btnUpdate.click();
    await ProfilePage.successMsg.waitForExist({ timeout: 3000 });

    await expect(ProfilePage.successMsg).toHaveText("Profile updated successfully");
